...
(function ($) {
    ...
    $.fn.validation = function (settings) {
        ...
        $(this).find("[validator]").find("input[type=radio]").each(function () {
            if (!this.disabled) {
        ...
        if (settings.inlineValidation == true) { // Validating Inline ?
            if (!settings.returnIsValid) { // NEEDED FOR THE SETTING returnIsValid
                ...
                if (settings.liveEvent) { // LIVE event, vast performance improvement over BIND
                    $(this).find("[validator][type!=checkbox]").live(settings.validationEventTriggers, function (caller) {
                        _inlinEvent(this);
                    })
                    $(this).find("[validator][type=checkbox],[validator][type=radio]").live("click", function (caller) {
                        _inlinEvent(this);
                    })
                } else {
                    $(this).find("[validator]").not("[type=checkbox]").bind(settings.validationEventTriggers, function (caller) {
                        _inlinEvent(this);
                    })
                    $(this).find("[validator][type=checkbox],[validator][type=radio]").bind("click", function (caller) {
                        _inlinEvent(this);
                    })
                }
                //下拉框添加校验到改变
                $(this).find("select[validator]").live("change", function (caller) {
                    _inlinEvent(this);
                });

                $(this).find("input[validator][type=text]").each(function () {
                ...
                $(this).find("textarea[validator]").each(function () {
                    var event = $(this).attr("event");
                    if (event) {
                        $(this).bind(event, function (caller) {
                            _inlinEvent(this);
    ...
    $.validation = {
        ...
        validateCall: function (caller, rules) { // EXECUTE VALIDATION REQUIRED BY THE USER FOR THIS FIELD
            ...
            function radioHack() {
                if ($("input[name='" + callerName + "']").size() > 1 && (callerType == "radio" || callerType == "checkbox")) { // Hack for radio/checkbox group button, the validation go the first radio/checkbox of the group
                    caller = $("input[name='" + callerName + "'][type!=hidden]");
                }
            }
        ...
        buildPrompt: function (caller, promptText) { // ERROR PROMPT CREATION AND DISPLAY WHEN AN ERROR OCCUR
            ...
            //$(caller).attr("errorInfo", promptText).parent("td").append("<span class='tipsy-container tipsy-error' title='"+promptText+"'></span>") ;
        ...
        validate: function (caller) { // FORM SUBMIT VALIDATION LOOPING INLINE VALIDATION
            var stopForm = false;
            var errorInfo = "";
            $(caller).find("[validator]").each(function () {
                var validationPass = $.validation.loadValidation(this);
                return (validationPass) ? stopForm = true : "";
            });
            if (stopForm) {
                $(caller).find("[errorInfo]").each(function () {
                    errorInfo += $.trim(($(this).parents("td").eq(0).prev("th").text() || '').replace('*', ''));
                    errorInfo += $.trim($(this).attr("errorInfo")) + "\n";
                });
                //焦点在第一个上面
